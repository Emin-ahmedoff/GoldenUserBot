# Copyright (C) 2021 The Projects Company LLC.
#
# Licensed under the Golden Projects Public License, Version 1.c (the "License");
# you may not use this file except in compliance with the License.
#

# Thanks github.com/spechide for creating inline bot support.
# Golden UserBot - Erdem By - ByMisakiMey
""" UserBot hazırlanışı. """
...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        level=DEBUG,
        format="[%(asctime)s - %(levelname)s] - @GoldenUserBot : %(message)s",
        datefmt='%d-%b-%y %H:%M:%S')
else:
    basicConfig(
        level=INFO,
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/Emin-ahmedoff/GoldenUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/Emin-ahmedoff/GoldenUserBot.git" #if not STABILITY else https://github.com/Emin-ahmedoff/GoldenUserBot.git Eski Dostum Berce'ye Aittir.
EMERGENCY = "https://github.com/erdewbey/GoldenUserBot.git" # Acil durrum için
...
if not BLACKLIST_CHAT: #Eğer ayarlanmamışsa Golden Support grubu eklenir.
...
with bot:
    ...
    try:
        ...
        bot(JoinChannelRequest("@goldenplugin"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @GoldenUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Golden açabilirsin; Kanala bak` @GoldenUserBot')
            else:
                await event.reply(f'`Tengri save Turks! Golden working... `')
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@GoldenUserBot":
                ...
                result = await builder.article(
                    ...
                    text=f"**En Gelişmiş UserBot!** [Golden](https://t.me/GoldenUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@GoldenUserBot",
                    text="""@GoldenUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/GoldenUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/GoldenSupportaz")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Emin-ahmedoff/GodlenUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @GoldenUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Golden](https://t.me/GoldenUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
...
async def autobot():
    ...
    if usnm:
        ...
        username = "golden_" + (str(uid))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if not isdone.startswith("Good."):
        await bot.send_message(bf, "My Golden Bot")
        time.sleep(3)
        isdone = (await bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.info(
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "golden_" + (str(uid))[6:] + str(ran) + "_bot"
        await bot.send_message(bf, username)
        time.sleep(3)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            time.sleep(3)
            await bot.send_file(bf, "image/golden.jpg") 
            heroku_var["BOT_TOKEN"] = token
            heroku_var["BOT_USERNAME"] = username
            LOGS.info(f"GoldenUserBot ile otomatik olarak asistan oluşturuldu. @{username}")
        else:
            LOGS.info(
        ...
        await bot.send_message(bf, f"@{username}")
        time.sleep(3)
        await bot.send_message(bf, "@GoldenUserBot ile otomatik olarak oluşturulmuş Asistan")
        time.sleep(3)
        await bot.send_message(bf, "/setuserpic")
        time.sleep(3)
        await bot.send_message(bf, f"@{username}")
