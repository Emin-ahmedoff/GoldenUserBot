...
def get_spotify_info(TIME=5):
    try:
        ...
        if len(item['album']['images']) > 0:
            ...
            if path.exists("@GoldenUserBot-Spotify.jpg"):
                remove("@GoldenUserBot-Spotify.jpg")          
            try:
                r = get(str(item['album']['images'][0]['url']))
                with open("@GoldenUserBot-Spotify.jpg", 'wb') as f:
                    f.write(r.content)    

                with open('@GoldenUserBot-Spotify.jpg', 'rb') as f:
