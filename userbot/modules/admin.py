...
"""
Qrup yönetmenize yardımcı olacak UserBot modülü
"""
        ...
        if not event.is_channel and event.is_group:
            for user_id in to_add_users.split(" "):
                ...
                try:
                    ...
                    await event.edit(f'`{user_id} qrupa eləvə edilmədi!`')
                    continue
                await event.edit(f'`{user_id} qruba elavə edildi!`')
        else:
            for user_id in to_add_users.split(" "):
                await event.edit(f'`{user_id} qrupa əlavə edilir...`')
                try:
                    ...
                    await event.edit(f'`{user_id} qrupa əlavə etmək mümkün olmadı!`')
                    continue
                await event.edit(f'`{user_id} qrupa əlavə edildi!`')

@register(outgoing=True, pattern="^.gban(?: |$)(.*)")
async def gbanspider(gspdr):
    """ .gban əmri göstərilən şəxsi qlobal olaraq qadağan edir """
    # Yetki kontrolü
...
async def gbanmsg(moot):
    """ Qlobal olaraq qadağan edilmiş istifadəçi mesaj alırsa """
    try:
...
async def ungban(un_gban):
    """ .ungban əmri göstərilən şəxsin qlobal səssizliyini aradan qaldırır """
    # Yetki kontrolü
...
async def set_group_photo(gpic):
    """ .Qrupunuzun şəklini setgpic əmri ilə dəyişə bilərsiniz. """
...
async def promote(promt):
    """ .promote Komanda ilə göstərilən şəxsi idarəçi edir. """
    # Hedef sohbeti almak
    ...
    if not rank:
        rank = "İcraçı"  # Her ihtimale karşı.
    if user:
        pass
    else:
        return
...
async def demote(dmod):
    """ .demote əmr müəyyən edilmiş şəxsi adminlikdən çıxarır """
    # Yetki kontrolü
...
async def ban(bon):
    """ .ban komanda göstərilən şəxsi qrupdan qadağan edir """
    # Yetki kontrolü
...
async def nothanos(unbon):
    """ .unban əmr göstərilən şəxsin qadağanını aradan qaldırır """
    # Yetki kontrolü
...
async def spider(spdr):
    """
    Bu funksiya əsasən səsi söndürmək üçündür.
    """
    # Fonksiyonun SQL modu altında çalışıp çalışmadığını kontrol et
    try:
...
async def unmoot(unmot):
    """ .unmute əmr göstərilən şəxsin səsini açır (yəni qrupda yenidən danışa bilər) """
    # Yetki kontrolü
...
async def muter(moot):
    """ Səssiz istifadəçilərin mesajlarını silmək üçün istifadə olunur """
    try:
...
async def ungmoot(un_gmute):
    """ .ungmute əmr göstərilən şəxsin qlobal səssizliyini aradan qaldırır """
    # Yetki kontrolü
...
async def gspider(gspdr):
    """ .gmute əmr qlobal olaraq təyin edilmiş şəxsi səssizləşdirir """
    # Yetki kontrolü
    chat = await gspdr.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
async def rm_deletedacc(show):
    """ .zombies əmri söhbətdəki bütün xəyal/silinmiş/zombi hesablarını siyahıya alır. """
    ...
    if BOTLOG:
        await show.client.send_message(
            ...
            f"**{del_u}** ədəd silinmiş hesap çıxarıldı !!\
            \nGRUP: {show.chat.title}(`{show.chat_id}`)")


@register(outgoing=True, pattern="^.admins$")
async def get_admin(show):
    """ .admins Daxil edilmiş qrupun administratorlarını sadalayır """
    ...
    try:
        async for user in show.client.iter_participants(
            ...
            if not user.deleted:
                ...
                mentions += f"\nSilinmiş Hesab <code>{user.id}</code>"
...
async def pin(msg):
    """ .pin Komandanın verildiyi qrupdakı mətni və medianı sabitləyər """
    # Yönetici kontrolü
...
async def kick(usr):
    """ .kick komanda göstərilən şəxsi qrupdan çıxarır """
    # Yetki kontrolü
...
async def get_users(show):
    """ .users Komanda ilə daxil edilmiş qrupa aid kontaktları sadalayır """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    mentions = '{} qrupdakı insanlar: \n'.format(title)
    try:
    ...
    try:
        ...
        await show.edit(
            "Lənət olsun, bu böyük bir qrupdur. İstifadəçi siyahısını fayl olaraq göndərirəm.")
        ...
        await show.client.send_file(
            show.chat_id,
            "userslist.txt",
            caption='{} qrupdakı insanlar'.format(title),
            reply_to=show.id,
        )
...
async def get_user_from_event(event):
    """ İstifadəçini mübahisədən və ya cavab mesajından əldə edin. """
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if not user:
            await event.edit("`Şəxsin istifadəçi adını, idini və ya cavabını ötürün!`")
            return
...
async def unwarn(event):
    """ .unwarn istifadəçi xəbərdarlığını aradan qaldırmaq üçün işləyir """
    # Yetki kontrolü
...
async def warn(event):
    """ .warn istifadəçini xəbərdar etmək üçün faydalıdır """
    # Yetki kontrolü
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
async def Warn_Gmute(event, warn, user, reason = None):
    await event.delete()
    yeni = await event.reply(f"`Sizə kifayət qədər xəbərdarlıq etdim` [{user.first_name}](tg://user?id={user.id})`, siz qlobal səviyyədə susdurulursunuz!`")
    ...
    if gmute(user.id) == False:
        await yeni2.edit(
            '`Səhv! İstifadəçi artıq qlobal olaraq səssizdir.`')
    else:
        if reason != None:
            await yeni2.edit(f"`İstifadəçi qlobal miqyasda səsi kəsildi!`Səbəb: {reason}")
        else:
            await yeni2.edit("`İstifadəçi qlobal olaraq səssizləşdirildi!`")
...
async def Warn_Gban(event, warn, user, reason = None):
    await event.delete()
    yeni = await event.reply(f"`Sizə kifayət qədər xəbərdarlıq etdim` [{user.first_name}](tg://user?id={user.id})`, dünya miqyasında qadağan olunursunuz!`")
    ...
    if gban(user.id) == False:
        await yeni2.edit(
            '`Səhv! İstifadəçi artıq qlobal şəkildə qadağan edilib.`')
    else:
        if reason != None:
            await yeni2.edit(f"`İstifadəçi qlobal miqyasda qadağan edilib!`Səbəb: {reason}")
        else:
            await yeni2.edit("`İstifadəçi qlobal şəkildə qadağandır!`")
...
async def get_usersdel(show):
    """ .usersdel komanda qrupda silinmiş hesabları göstərir """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    ...
    try:
        ...
        await show.edit(
            "Lənət olsun, bu böyük bir qrupdur. Silinmiş istifadəçilərin siyahısını fayl olaraq göndərirəm.")
        file = open("userslist.txt", "w+")
        file.write(mentions)
        file.close()
        await show.client.send_file(
            ...
            caption='{} qrupa aid silinmiş hesablar:'.format(title),
            reply_to=show.id,
        )
...
async def get_userdel_from_event(event):
    """ Arqumentdən və ya cavablandırılmış mesajdan silinmiş istifadəçini bərpa edin. """
    args = event.pattern_match.group(1).split(' ', 1)
    extra = None
    if event.reply_to_msg_id and not len(args) == 2:
        previous_message = await event.get_reply_message()
        ...
        if not user:
            await event.edit("`Silinmiş istifadəçinin istifadəçi adını, idini və ya cavabını ötürün!`")
            return
...
async def get_bots(show):
    """ .bots komanda qrupa aid olan botları siyahıya alır """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    ...
    try:
        ...
        await show.edit(
            "Lənət olsun, burada çoxlu botlar var. Botların siyahısını fayl olaraq göndərirəm.")
...
CmdHelp('admin').add_command(
        'promote', '<istifadəçi adı/cavab> <xüsusi ad (istəyə bağlı)>', 'Söhbətdəki şəxsə admin hüquqları verir.'
    ).add_command(
        'demote', '<username/reply>', 'Söhbətdəki şəxsin admin icazələrini ləğv edir.'
    ).add_command(
        'ban', '<username/reply> <səbəb (istəyə bağlı)>', 'Söhbətdəki şəxsi susdurun, adminlər üçün də işləyir.'
    ).add_command(
        'unban', '<username/reply>', 'Söhbətdəki şəxsi qadağadan çıxarır.'
    ).add_command(
        'kick', '<istifadəçi adı/cavab> <səbəb (isteğe bağlı)>', 'Qrupda qeyd etdiyiniz şəxsi təpikləyir.'
    ).add_command(
        'gmute', '<username/reply> <səbəb (istəyə bağlı)>', 'İdarəçi olduğunuz bütün qruplarda kontaktın səsini söndürün.'
    ).add_command(
        ...
        'warn', '<istifadəçi adı/cavab verməyin> <səbəb (isteğe bağlı>', 'Göstərdiyiniz istifadəçini xəbərdar edir.'
    ).add_command(
        ...
        'gban', '<username/reply>', 'İstifadəçini qlobal olaraq bloklayır.'
    ).add_command(
        'ungban', '<username/reply>', 'İstifadəçinin qlobal qadağasını aradan qaldırır.'
    ).add_command(
        'pin', '<reply>', 'Cavab verdiyiniz mesajı sancaqlayır.'
    ).add_command(
        'setgpic', '<reply>', 'Qrup şəklini dəyişdirir.'
    ).add()