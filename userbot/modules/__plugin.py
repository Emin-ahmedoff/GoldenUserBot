    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Plugin uğurla yükləndi!**\n__Modulun əmrləri və istifadəsi haqqında məlumat əldə etmək üçün__ `.golden {cmdhelp}` __yazınız.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Plugin uğurla yükləndi!**\n__Modulun əmrləri və istifadəsi haqqında məlumat əldə etmək üçün__ `.golden {dosyaAdi}` __yazınız.__')
...
async def psend(event):
    ...
    if os.path.isfile(f"./userbot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./userbot/modules/{modul}.py", caption=LANG['GOLDEN_PLUGIN_CAPTION'])
        await event.delete()
    else:
        await event.edit(LANG['NOT_FOUND_PLUGIN'])
...
async def ptest(event):
    ...
    return await event.edit(f'**Plugin uğurla Silindi!**\
