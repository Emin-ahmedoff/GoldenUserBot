...
from userbot import CMD_HELP, HEROKU_APIKEY, HEROKU_APPNAME, UPSTREAM_REPO_URL, SAHIB
...
async def seden_update(ups):
    ...
    if ups.is_reply:
        ...
        if ren == SAHIB:
            ...
            if not changelog and not force_update:
                await usp.edit(LANG['EMİNUPDATE'].format(ac_br))
                repo.__del__()
                return
            ...
            if HEROKU_APIKEY is not None:
                ...
                if heroku_app is None:
                    ...
                    repo.__del__()
                    return
                await usp.edit(LANG['EMİN_UPDATING'])
                ups_rem.fetch(ac_br)
                repo.git.reset("--hard", "FETCH_HEAD")
                heroku_git_url = heroku_app.git_url.replace(
                    "https://", "https://api:" + HEROKU_APIKEY + "@")
                ...
                try:
                    ...
                    repo.__del__()
                    return
                await usp.edit(LANG['EMİNSUCCESSFULLY'])
            else:
                try:
                    ups_rem.pull(ac_br)
                except GitCommandError:
