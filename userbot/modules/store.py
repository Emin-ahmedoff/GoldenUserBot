...
async def magaza(event):
    ...
    await event.edit('**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n`🔎 Plugin\'i axtarıram... Biraz gözlə.`')
    split = plugin.split()
    if plugin == '':
        plugin = 'Son Yüklənən'
        plugins = await event.client.get_messages('@goldenplugin', limit=15, filter=InputMessagesFilterDocument)
    elif len(split) >= 1 and (split[0] == 'random' or split[0] == 'rastgele'):
        plugin = 'Rastgele'
        plugins = await event.client.get_messages('@goldenplugin', limit=None, filter=InputMessagesFilterDocument)
        plugins = sample(plugins, int(split[1]) if len(split) == 2 else 5)
    else:
        plugins = await event.client.get_messages('@goldenplugin', limit=None, search=plugin, filter=InputMessagesFilterDocument)
        random = await event.client.get_messages('@goldenplugin', limit=None, filter=InputMessagesFilterDocument)
        random = choice(random)
        random_file = random.file.name

    result = f'**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n**🔎 Axtarış:** `{plugin}`\n**🔢 Nəticələr: __({len(plugins)})__**\n➖➖➖➖➖\n\n'
...
async def sinstall(event):
    ...
    try:
        ...
        return await event.edit('**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n**⚠️ Xəta:** `Zəhmət olmasa sadəcə rəqəm yazın. Əgər plugin axtarmaq istəyirsinizsə .store komandasını işlədin.`')
    
    await event.edit('** 🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n`🔎 Plugin\'i yükləyirəm... Biraz gözləyin.`')
    plugin = await event.client.get_messages('@goldenplugin', ids=plugin)
    await event.edit(f'**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n`✅ {plugin.file.name} yükləmə uğurludur!`\n`⬇️ Plugini yükləyirəm... Biraz gözləyin.`')
    ...
    try:
        ...
        return await event.edit(f'**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n**⚠️ Xəta:** `Plugin xətalıdır. {e}`\n**BUNU ADMİNLƏRƏ DEYİN!**')
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modul uğurla yükləndi!**\n__Modul haqqında məlumat üçün__ `.golden {cmdhelp}` __yazın.__')
            else:
                await plugin.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                cmdhelp = re.findall(r"CmdHelp\([\"'](.*)[\"']\)", dosy)[0]
                await plugin.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n**✅ Modul uğurla yükləndi!**\n__ℹ️ Modul haqqında məlumat üçün__ `.golden {cmdhelp}` __yazın.__')
            else:
                dosyaAdi = plugin.file.name.replace('.py', '')
                extractCommands(dosya)
                await plugin.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**🪙 Golden Plugin Mağazası**\n__Versiyon 1.0__\n\n**✅ Modül başarıyla yüklendi!**\n__ℹ️ Modul haqqında məlumat üçün__ `.golden {dosyaAdi}` __yazın.__')
