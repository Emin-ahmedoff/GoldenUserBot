...
async def shazam(event):
    if not event.is_reply:
        ...
        await event.edit('`Audio faylını yükləyirəm...👀`')
        ...
        if 'artists' in sarki['track']:
            Caption += f'**İfaçı(lər):** [{sarki["track"]["subtitle"]}](https://www.shazam.com/artist/{sarki["track"]["artists"][0]["id"]})\n'
        else:
...
CmdHelp('shazam').add_command(
    'shazam', '<cavab>', "Cavab verdiyiniz Səs'i Shazamda axtarar."
).add()