...
async def sleepybot(time):
    """ .sleep komandası GoldenUserBot'u yatızdırar :) """
...
async def creator(e):
    await e.edit("<b>GOLDEN UserBot @emin_ehmedoff tərəfindən yaradılıb.</b>",
                 parse_mode='html')


@register(outgoing=True, pattern="^.readme$")
async def reedme(e):
    await e.edit("https://github.com/Emin-ahmedoff/GoldenUserBot#readme",
                 parse_mode='html')
...
async def repo(repo):
    ...
    await repo.edit('[GOLDEN\'s repository](https://github.com/Emin-ahmedoff/GoldenUserBot)')
...
CmdHelp('misc').add_command(
    ...
    'sleep', '<vaxt>', 'GoldUserBot\'u yazdığınız saniyə qədər yatızdırar', 'sleep 20'
).add_command(
    'shutdown', None, 'Botu Söndürər.'
).add_command(
    'repo', None, 'GoldUserBot\'un GitHub reposunun linki.'
).add_command(
    'readme', None, 'GoldenUserBot\'un GitHub\'dakı README.md faylının linki.'
).add_command(
